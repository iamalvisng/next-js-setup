...
export default function Home() {
  const achorRef = createRef<HTMLAnchorElement>();
  const dispatch = useDispatch();
  const counterValue = useSelector(selectCounterValue);
  const [name] = useState<string>('pikachu');
  const [shouldAuth] = useState(false);
  const [form] = useStateReducer<TForm>({ email: '', password: '' });
      ...
      <section className="bg-white dark:bg-gray-800 flex justify-center p-8">
        <div className="container">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button ref={achorRef} onClick={onIncrease} text="Increment" />
            <Button ref={achorRef} onClick={onDecrease} text="Decrement" />
          </div>

          <div className="flex justify-center my-8">
            <Link passHref href={`/pokemon?name=${name}`}>
              <Button ref={achorRef} onMouseOver={() => prefetch(name)} text="View Details" />
            </Link>
          </div>
        </div>
      </section>
